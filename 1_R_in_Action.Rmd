...
```{r}
library(ggplot2) # package, not a library (library is a command)

head(diamonds)
nrow(diamonds)
str(diamonds) # tibble comes from dplyr package
summary(subset(diamonds, select = c(price, carat, color, clarity, cut)))

# Univariate plots 
# chartreues4 is a color
hist(diamonds$price, breaks = "FD", col = "chartreuse4")
# right-skewed distribution
hist(diamonds$carat, breaks = "FD", col = "chartreuse4")
# right-skewed too, less regular (why do we have some peaks? diamonds are cut e.g. 1 carat)
...
plot(
  ...
  col = "chartreuse4", 
  pch = ".", 
  log = "xy", # here we set the log scale
  main = "log-log scale"
)

# logx and logy -> linear, the bigger, the more expensive
...
# why better clarity less price? confounding effect?
...
-   "carat" and "price" are numeric variables, while "color" is an ordered categorical with categories D \< E \< F \< G \< H \< I \< J (D is the best color). "cut" is also ordered with categories Fair \< Good \< Very Good \< Premium \< Ideal, and "clarity" is ordered with categories I1 \< SI2 \< SI1 \< VS2 \< VS1 \< VVS2 \< VVS1 \< IF (best, internally flawless).
...
# (no overhead, exactly what we wanted to do)
quote(iris |> head(2))

# plus is a function
`+`(1,1)
`-`<-function(e,f){print('hahaha')}
rm(`-`)
```
...
(input is a dataframe, output is a dataframe, so we can use pipes) (we can do everything in Base R as well, but it's not that convenient)
...
diamonds %>%
  arrange(-price) %>% # ascending/descending
  select(price, carat) %>% 
  head(2)
...
# sapply(diamonds[c("carat", "price")],  FUN="median")
...
diamonds %>%
  ...
  left_join(med, by = "clarity") %>% # TODO: why left_join?
  head(2)
...
Core idea: use plus to add a layer by layer

Why no pipe? older than the pipe?
...
Never change something in the "parent environment"!
...
Integers bigger than 2\^50 -\> converted to a floating point number with precision -\> algorithm will fail. So for cryptographic uses, we need to use packages that allow bigger precise integers. In Python, we don't have this issues.

We don't do this for analysis (but for packages).
...
gcd <- function(a, b) {
  stopifnot(
    "comment why" = length(a) == 1, 
    "provide a number" = length(b) == 1,
    a >= 1,
    b >= 1,
    a == trunc(a),
    b == trunc(b),
...
class(price ~ color)
# standard plot (not from ggplot2)
# what is going on: plot finds the class of 'price ~ color' and calls plot.formula 
# i.e. graphics:::plot.formula function (::: for internal functions)
...
# two classes, here plot.factor is used
plot(diamonds$color)
```
