...
- The case $n=1$ simply shows the histogram corresponding to a standard exponential distribution. It is very asymmetric and non-normal. (translated by -true mean)
- Note that the sum of exponential random variables is Gamma distributed.

replicate is not faster than for loop, but it looks nicer, e.g. `replicate(2, "hello")`
...
Purple intervals are the "wrong" ones, which do not contain the true parameter 1. Here we see what 95% means: If we have 100 samples, around 95 contain the true parameter.
...
- "BCa": Bias-corrected and accelerated. In theory the most accurate of the four, and thus in practice often the recommended type of interval. (even for smaller sample size, it will converge faster), choose this one if it is used for publication
...
Never claim: H_0 is true, only the other way around (as in the courthouse)
...
which Type 1 error is closer to 5? (anticonservative)
power? 1-Type 2 error (?)

repeated sampling -> compare two in two dimension
if we have just one sample (as in reality) -> not possible
